...
public class IgnoreNBTIngredient extends Ingredient {
	...
	protected IgnoreNBTIngredient(final ItemStack... stacks) {
		super(0);
	...
	public boolean apply(@Nullable final ItemStack stackToCheck) {
		...
		for (final ItemStack stack : matchingStacks) {
			if (stack.getItem() == stackToCheck.getItem()) {
				...
				if ((metadata == OreDictionary.WILDCARD_VALUE || ItemStack.areItemsEqualIgnoreDurability(stackToCheck, stack))) {
					return true;
				}
			}
		}
